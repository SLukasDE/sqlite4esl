...
sqlite3& Driver::open(const std::string connectionString) const {
	...
	if(db == nullptr) {
		throw esl::stacktrace::Stacktrace::add(std::runtime_error("SQLite is unable to allocate memory to open database \"" + connectionString + "\""));
	}
	...
	if(rc != SQLITE_OK) {
        ...
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(message));
	}
...
void Driver::close(const ConnectionFactory& connectionFactory) const {
	...
	if(rc != SQLITE_OK) {
		...
		if(rc != SQLITE_OK) {
	        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Cannot close database connection: ") + sqlite3_errstr(rc)));
		}
	}
}
...
StatementHandle Driver::prepare(const Connection& connection, const std::string& sql) const {
	...
	if(rc != SQLITE_OK) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Can't prepare SQL statement \"" + sql + "\": ") + sqlite3_errstr(rc)));
	}
...
void Driver::finalize(StatementHandle& statementHandle) const {
	...
	if(rc != SQLITE_OK) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Can't close statement handle: ") + sqlite3_errstr(rc)));
	}
}
...
int Driver::step(StatementHandle& statementHandle) const {
	...
	case SQLITE_ROW:
		break;
	case SQLITE_BUSY:
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Cannot fetch, because sqlite3_step returned SQLITE_BUSY: ") + sqlite3_errstr(rc)));
	case SQLITE_MISUSE:
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Cannot fetch, because sqlite3_step returned SQLITE_MISUSE: ") + sqlite3_errstr(rc)));
	case SQLITE_ERROR:
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Cannot fetch, because sqlite3_step returned SQLITE_ERROR: ") + sqlite3_errstr(rc)));
	default:
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("Cannot fetch, because sqlite3_step returned " + std::to_string(rc) + ": " + sqlite3_errstr(rc)));
	}
...
void Driver::reset(StatementHandle& statementHandle) const {
	...
	if(rc != SQLITE_OK) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error(std::string("Can't reset statement handle: ") + sqlite3_errstr(rc)));
	}
}
...
std::size_t Driver::columnCount(StatementHandle& statementHandle) const {
	...
	if(count < 0) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("sqlite3_column_count returned a negative value: " + std::to_string(count)));
	}
...
std::string Driver::columnName(StatementHandle& statementHandle, std::size_t index) const {
	...
	if(name == nullptr) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("sqlite3_column_name returned a null pointer for index " + std::to_string(index)));
	}
...
std::string Driver::columnText(StatementHandle& statementHandle, std::size_t index) const {
	...
	if(data == nullptr) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("sqlite3_column_text returned a null pointer"));
	}

	int length = sqlite3_column_bytes(&statementHandle.getHandle(), static_cast<int>(index));
	if(length < 0) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("sqlite3_column_bytes returned a negative value: " + std::to_string(length)));
	}
...
std::size_t Driver::bindParameterCount(StatementHandle& statementHandle) const {
	...
	if(count < 0) {
        throw esl::stacktrace::Stacktrace::add(std::runtime_error("sqlite3_bind_parameter_count returned a negativ value: " + std::to_string(count)));
	}
